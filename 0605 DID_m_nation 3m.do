...
import delimited "0606-df_user_nation_treat_3m.csv", case(preserve) clear
encode version_User_Id, generate(country_id)
gen policy = month_nb - 5
...
logout,save(img3m/descriptive_statistics)word replace:tabstat ln_Score ln_pure_string_len pure_body_smog_index full_code_string_ratio tt ln_TotalComment ln_AnswerComment ln_QuestionerComment ln_ViewerComment ln_Q_Tags_len_filter std_Q_Tags_len_filter median_Q_Tags_len_filter $control_fe,s(N mean sd min max) f(%12.3f) c(s)
// swilk ln_body_string_len
// qnorm full_code_string_ratio

wmtcorr ln_Score ln_pure_string_len pure_body_smog_index full_code_string_ratio tt ln_TotalComment ln_AnswerComment ln_QuestionerComment ln_ViewerComment ln_Q_Tags_len_filter std_Q_Tags_len_filter median_Q_Tags_len_filter $control_fe using img3m/correlationship.rtf, replace
...
scheme(s1mono)
graph export "img3m/Parallel ln_Score.svg", replace
...
msymbol(circle_hollow) /// 
scheme(s1mono)
graph export "img3m/Parallel de_ln_pure_string_len.svg", replace
...
msymbol(circle_hollow) /// 
scheme(s1mono)
graph export "img3m/Parallel de_pure_body_smog_index.svg", replace
...
msymbol(circle_hollow) /// 
scheme(s1mono)
graph export "img3m/Parallel de_full_code_string_ratio.svg", replace
graph save pt4, replace
drop b_pre_* avg_coef

graph combine pt1.gph pt2.gph pt3.gph pt4.gph, col(2)
graph export "img3m/Parallel_pans.svg", replace 
...
est store m14
esttab m11 m12 m13 m14 using "img3m/DIDRegression_model_3m.rtf",se replace 

***moderate ln_Q_Tags_len_filter
...
reghdfe ln_Score c.tt##c.std_Q_Tags_len_filter  $control_fe, a(country_id month_nb) vce(cluster country_id)
est store mw32std
esttab mw31std mw311std mw32std using img3m/DID_moderate_Q_Tags_len_filter_std.rtf,se replace 
...
graph export "img3m/moderate flenmw32std.jpg", replace
...
est store m110
esttab m13 m18 m19 m110 using "img3m/DID_3m_comment.rtf",se replace 
...
esttab mw1 mw2 using img3m/DID_3m_moderate_group.rtf, se replace

//*******************************placebo**************************************
gen did=treat*after

reghdfe ln_Score did , a(country_id month_nb nation) vce(cluster country_id)
cap erase "ln_Score simulations.dta"
permute did beta = _b[did] se= _se[did] df = e(df_r), reps(500) rseed(42) saving("ln_Score simulations.dta"):reghdfe ln_Score did, a(country_id month_nb nation) vce(cluster country_id)

reghdfe ln_pure_string_len did , a(country_id month_nb nation) vce(cluster country_id)
cap erase "ln_pure_string_len simulations.dta"
permute did beta = _b[did] se= _se[did] df = e(df_r), reps(500) rseed(42) saving("ln_pure_string_len simulations.dta"):reghdfe ln_pure_string_len did, a(country_id month_nb nation) vce(cluster country_id)


reghdfe pure_body_smog_index did , a(country_id month_nb nation) vce(cluster country_id)
cap erase "pure_body_smog_index simulations.dta"
permute did beta = _b[did] se= _se[did] df = e(df_r), reps(500) rseed(42) saving("pure_body_smog_index simulations.dta"):reghdfe pure_body_smog_index did, a(country_id month_nb nation) vce(cluster country_id)

reghdfe full_code_string_ratio did , a(country_id month_nb nation) vce(cluster country_id)
cap erase "full_code_string_ratio simulations.dta"
permute did beta = _b[did] se= _se[did] df = e(df_r), reps(500) rseed(42) saving("full_code_string_ratio simulations.dta"):reghdfe full_code_string_ratio did, a(country_id month_nb nation) vce(cluster country_id)

***plot
use "ln_Score simulations.dta", clear
gen t_value= beta/se
gen p_value=2*ttail(df, abs(beta/se))
dpplot beta, xline(0.0184,lp(dash)) xtitle("Estimator") ytitle("Density") ///
    text(-5 0.0184 "0.0184" )
graph save placet31, replace

graph export "img3m/placebo-ln_Score.png", replace
